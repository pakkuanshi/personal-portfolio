...
function ContactValue({ item }: { item: ContactItem }) {
  if (isLinkItem(item)) {
    return (
      <Link
        className="inline-flex max-w-full break-words font-serif text-[clamp(1.28rem,2vw,1.86rem)] leading-[1.08] tracking-normal text-[hsl(220_18%_12%/0.9)] underline decoration-transparent underline-offset-4 transition-colors duration-200 hover:text-[hsl(28_24%_38%)] hover:decoration-current dark:text-[hsl(42_28%_90%/0.9)] dark:hover:text-[hsl(42_30%_96%)]"
        href={item.href}
  ...
  if (Array.isArray(item.value)) {
    return (
      <div className="grid gap-1 font-serif text-[clamp(1.28rem,2vw,1.86rem)] leading-[1.12] tracking-normal text-[hsl(220_18%_12%/0.88)] dark:text-[hsl(42_28%_90%/0.88)]">
        {item.value.map((line) => (
          <p key={line}>{line}</p>
        ))}
      </div>
  ...
  return (
    <p className="max-w-xl font-serif text-[clamp(1.28rem,2vw,1.86rem)] leading-[1.12] tracking-normal text-[hsl(220_18%_12%/0.88)] dark:text-[hsl(42_28%_90%/0.88)]">
      {item.value}
    </p>
  );
}
      ...
      <div className="site-container grid min-h-[calc(100svh-3.25rem)] content-center py-14 md:py-20">
        <div className="grid gap-14 lg:grid-cols-[minmax(0,1.36fr)_minmax(18rem,1fr)] lg:gap-16 xl:gap-24">
          <header className="max-w-4xl">
            ...
            <h1 className="mt-5 max-w-4xl font-serif text-[clamp(3rem,7vw,7rem)] font-medium leading-[0.95] tracking-normal text-[hsl(220_18%_12%/0.94)] dark:text-[hsl(42_28%_90%/0.94)]">
              {copy.title}
            </h1>
